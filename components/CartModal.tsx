...
interface CartModalProps {
  isOpen: boolean;
  onClose: () => void;
  charms: Charm[]; // We can use this to show dynamic items if needed, or stick to static for now
}

export const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose, charms }) => {
  if (!isOpen) return null;

  // Combining sample static items with user generated charms for visual fullness
  const cartItems = [
    { id: 'static-1', name: 'Pandora Premium Collection', subtitle: 'Limited Edition Design', price: 299.99, type: 'item', colorFrom: '#60a5fa', colorTo: '#f9a8d4' },
    { id: 'static-2', name: 'AI Quiz Generator Pro', subtitle: 'Annual Subscription', price: 149.99, type: 'quiz', colorFrom: '#a78bfa', colorTo: '#f9a8d4' },
    ...charms.map(c => ({
        id: c.id,
        name: c.name,
        subtitle: 'Custom AI Forged Charm',
        price: 89.00,
        type: 'custom',
        colorFrom: c.color,
        colorTo: '#ffffff'
            ...
            <div className="space-y-4">
              {cartItems.map((item) => (
                <div key={item.id} className="flex gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                  ...
                  >
                    {item.type === 'custom' ? 'AI' : item.type === 'quiz' ? 'QUIZ' : 'ITEM'}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-medium text-gray-800 font-serif">{item.name}</h3>
                    <p className="text-sm text-gray-500 mt-1">{item.subtitle}</p>
                    <div className="flex items-center gap-3 mt-2">
                      <button className="w-7 h-7 rounded-full border border-gray-300 hover:bg-gray-200 flex items-center justify-center text-sm">-</button>
                  ...
                  <div className="text-right">
                    ...
                    <button className="text-red-500 text-sm mt-2 hover:text-red-600 transition-colors">Remove</button>
                  </div>
                </div>
              ))}
            </div>
